...
const TYPES = ['line', 'arrow', 'cloud', 'text', 'rect', 'flag', 'photo'];
  ...
  const markups = db
    .prepare(
      `SELECT m.*, u.name AS author_name FROM markups m
       ...
       WHERE m.document_id = ? AND (m.visibility = 'published' OR m.author_id = ?)
       ORDER BY m.created_at`
    )
    .all(req.params.documentId, req.session.user.id);
  res.json({
...
router.post('/', requireAuth, (req, res) => {
  ...
  const result = db
    .prepare(
      `INSERT INTO markups (document_id, author_id, visibility, type, geometry, style, linked_document_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
