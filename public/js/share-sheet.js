...
document.getElementById('download-share-sheet-btn').addEventListener('click', () => {
  openModal(`
    <h2>Download drawing</h2>
    <label class="permission-option">
      <input type="checkbox" id="dl-published" checked>
      <span><b>Published markups</b><small>Include markups published to this sheet.</small></span>
    </label>
    ${
      permissions.allow_personal_markups
        ? `<label class="permission-option">
            <input type="checkbox" id="dl-personal" checked>
            <span><b>Personal markups</b><small>Include your own private markups on this sheet.</small></span>
          </label>`
        : ''
    }
  ...
  document.getElementById('modal-ok').addEventListener('click', () => {
    ...
    const personalEl = document.getElementById('dl-personal');
    const includePersonal = permissions.allow_personal_markups && !!personalEl && personalEl.checked;
    const qs = new URLSearchParams({ published: includePublished ? '1' : '0', personal: includePersonal ? '1' : '0' });
    window.location.href = `/api/share/${token}/sheet-versions/${versionId}/download?${qs}`;
    closeModal();
  });
});
...
function setupPersonalTools() {
  ...
  async function start(evt) {
    ...
    if (activeTool === 'text') {
      ...
      if (text) await createPersonalMarkup('text', { x: pt.x / canvas.width, y: pt.y / canvas.height, text });
      return;
    }
    drawing = { start: pt };
    preview = el('rect');
